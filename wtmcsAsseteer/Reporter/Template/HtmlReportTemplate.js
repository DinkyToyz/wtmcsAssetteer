...
function initializeSingleShowHideToggleButtons(hideOnInit, autoHideButtons, idToggleButton, idShowButton, idHideButton, idHideable, classNamesShowElements, classNamesHideElements) {
    if (idToggleButton) {
        ...
        if (button) {
            button.href = 'javascript:showOrHideElement(' +
                quotedParameterString(idShowButton, !autoHideButtons) + ',' +
                quotedParameterString(idHideButton, !autoHideButtons) + ',' +
                quotedParameterString(idHideable) + ',' +
                quotedParameterString(classNamesShowElements) + ',' +
                quotedParameterString(classNamesHideElements) + ')';
...
function initializeSingleShowHideButtons(hideOnInit, autoHideButtons, idShowButton, idHideButton, idHideable, classNamesShowElements, classNamesHideElements) {
    ...
    if (idShowButton) {
        ...
        if (button) {
            button.href = 'javascript:showElements(' +
                quotedParameterString(idShowButton, !autoHideButtons) + ',' +
                quotedParameterString(idHideButton, !autoHideButtons) + ',' +
                quotedParameterString(idHideable) + ',' +
                quotedParameterString(classNamesShowElements) + ',' +
    ...
    if (idHideButton) {
        ...
        if (button) {
            button.href = 'javascript:hideElements(' +
                quotedParameterString(idShowButton, !autoHideButtons) + ',' +
                quotedParameterString(idHideButton, !autoHideButtons) + ',' +
                quotedParameterString(idHideable) + ',' +
                quotedParameterString(classNamesShowElements) + ',' +
            ...
            if (buttons) {
                for (i = 0; i < buttons.length; i++) {
                    if (buttons[i]) {
                        ...
                        if (id) {
                            buttons[i].href = 'javascript:showElements(' +
                                quotedParameterString(showButtonPrefix + id, !autoHideButtons) + ',' +
                                quotedParameterString(hideButtonPrefix + id, !autoHideButtons) + ',' +
                                quotedParameterString(hideableElementPrefix + id) + ')';
